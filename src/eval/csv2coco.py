...
def csv_to_coco(df: pd.DataFrame) -> Tuple[str, Dict]:
    ...
    def image(row):
        ...
        image["id"] = row.image_name[:-4]
        # image["id"] = image_id_map[row.image_name]
    ...
    def annotation(row):
        ...
        annotation = {}
        # annotation["image_id"] = image_id_map[row.image_name]
        annotation["image_id"] = row.image_name[:-4]
    ...
    imagedf = df.drop_duplicates(subset=['image_name']).sort_values(by='image_name')
